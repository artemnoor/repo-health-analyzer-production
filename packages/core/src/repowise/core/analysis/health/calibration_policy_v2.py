"""Shared Repo Health category calibration policies.

The calibration runner and production analyzers import the same pure
functions from this module.  It intentionally contains no repository I/O,
external-tool calls, or CategoryResult contracts: normalized facts are the
only inputs and every result is deterministic.
"""
...
def documentation_score(
    *,
    ...
    """Return the calibration-v2 documentation composition."""
    return clamp(
...
def activity_score(
    *,
    ...
    """Return calibrated activity score and its bounded components."""
...
def cicd_component_score(
    *,
    ...
    """Return calibrated CI score with explicit available-component reweighting."""
    components: dict[str, float] = {}
    if failure_rate is not None:
        components["reliability"] = anchored_quality(
            ratio(failure_rate) * 100.0,
            RELIABILITY_ANCHORS,
        )
