...
class Evidence(_ContractBase):
    ...
    _content_hash = field_validator("content_hash", "snippet_hash", mode="before")(
        lambda value, info: (
            None if value is None else _normalize_digest(value, field_name=info.field_name)
        )
    )
...
class Limitation(_ContractBase):
    ...
    _code = field_validator("code", mode="before")(
        lambda value: _normalize_key(value, field_name="code")
    )
...
class Metric(_ContractBase):
    ...
    _name = field_validator("name", mode="before")(
        lambda value: _normalize_key(value, field_name="name")
    )
    _evidence_ids = field_validator("evidence_ids", mode="before")(
        lambda value: _stable_ids(value, field_name="evidence_ids")
    )
...
class FactObservation(_ContractBase):
    ...
    _key = field_validator("key", mode="before")(
        lambda value: _normalize_key(value, field_name="key")
    )
    _evidence_ids = field_validator("evidence_ids", mode="before")(
        lambda value: _stable_ids(value, field_name="evidence_ids")
    )
...
class FactGroup(_ContractBase):
    ...
    def _stable_observations(self) -> FactGroup:
        object.__setattr__(
            self, "observations", tuple(sorted(self.observations, key=lambda item: item.key))
        )
        object.__setattr__(
...
class CategoryResult(_ContractBase):
    ...
    def _validate_result(self) -> CategoryResult:
        ...
        object.__setattr__(
            self, "findings", tuple(sorted(self.findings, key=lambda item: item.finding_id or ""))
        )
        object.__setattr__(
            self, "evidence", tuple(sorted(self.evidence, key=lambda item: item.evidence_id))
        )
        object.__setattr__(
            self, "limitations", tuple(sorted(self.limitations, key=lambda item: item.code))
        )
