...
class UnsupportedContractVersion(ContractValidationError):  # noqa: N818
...
class AnalysisRequest(_VersionedContract):
    ...
    _requested_analyzer_ids = field_validator("requested_analyzer_ids", mode="before")(
        _normalize_analyzer_ids
    )
    ...
    _as_of = field_validator("as_of", mode="after")(
        lambda value: _normalize_utc(value, field_name="as_of")
    )
